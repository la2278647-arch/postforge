...
同一篇 Markdown，输出十三个平台的成品：**微信公众号、知乎、掘金、CSDN、语雀、简书、博客园、思否、InfoQ、小红书、Medium、DEV.to、通用网页**。
...
- **MCP Server**：Claude / Cursor 可以直接调用 `build_post` / `check_post`
...
| 思否 | `-p sf` | HTML 片段 |
| InfoQ | `-p infoq` | HTML 片段 |
| Medium | `-p medium` | HTML 片段 |
| DEV.to | `-p devto` | HTML 片段 |
...
:::tip 图片怎么处理
公众号不支持外链图片：本地图片用 `--inline-images` 一键内联，粘贴时微信自动转存素材；外链图先下载再走素材库替换，详见 `docs/wechat-images.md`。
:::
