...
> Post title conventions on HN: "Show HN: ..." — keep the title short and concrete. Aligned with v0.6.1.

---

## Title

Show HN: PostForge – Markdown typesetting engine for 15 Chinese/international platforms (WeChat/Zhihu/Juejin/CSDN/Xiaohongshu…)

## Body

I built an open-source CLI that renders one Markdown file into publisher-ready rich text for the major Chinese content platforms — and now 15 platforms including Medium, DEV.to, Typecho and WordPress.
...
- Same command with `-p zhihu / -p juejin / -p csdn / -p yuque / -p medium / -p devto …` → rich text for that platform (15 in total)
...
- `postforge new` → scaffold drafts from 11 built-in templates (FAQ, weekly report, OKR, product launch, Xiaohongshu draft…)
...
- **10 themes**: clean / paper / dark / nord / coffee / midnight / one-dark / solarized / github-light / solarized-dark, extendable in `src/themes.js`
...
- **78 unit tests passing** (node:test), incl. CLI integration + MCP tests; CI matrix on Node 18/20/22/24
