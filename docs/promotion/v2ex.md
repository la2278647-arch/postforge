...
> V2EX 风格：标题 + 正文 + "回" 引导。发在 /t/ 技术节点。本版对齐 v0.6.5。

---

## 标题

[分享创造] PostForge：开源的 Markdown 多平台排版引擎（公众号/知乎/掘金/CSDN/小红书，16 平台）
...
- 一条命令 `postforge build post.md -p wechat`，Markdown → **16 平台**富文本（公众号/知乎/掘金/CSDN/语雀/简书/博客园/思否/InfoQ/小红书/Medium/DEV.to/Typecho/Hashnode/WordPress/通用网页）
- 全内联样式，粘贴即用，代码高亮（GitHub 配色）也全部内联化
- 支持 **11 套主题**（clean/paper/dark/nord/coffee/midnight/one-dark/solarized/github-light/solarized-dark/mint）、GFM 任务清单、表格斑马纹、目录生成（--toc）、标题自动编号
...
- **模板库**：`postforge new` 从 12 种模板一键生成草稿（FAQ/周报/OKR/产品发布/小红书种草/搬家清单等）
...
npm test 82 个用例全绿（含 CLI 集成与 MCP 测试），主流程已在全部 16 平台验证生成。
